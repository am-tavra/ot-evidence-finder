...
<h1>Seedling — Evidence Synthesis</h1>
...
<div class="footer">Generated by Seedling. Always verify against current literature and professional judgement. Not a substitute for clinical evaluation.</div>
</body></html>`;
...
export default function OTEvidencePlatform() {
  ...
  const accent = domainObj?.color || "#2D6A4F";
...
Provide 3-6 interventions ranked by evidence strength. Be specific and reference the articles by number.`;
      ...
      <div key={r.id || i} className="card-hover" style={{ background: r.isTrial ? "#F8F6FF" : "#fff", borderRadius: 10, border: marked ? `1.5px solid ${accent}55` : r.isTrial ? "1px solid #C5B8F0" : "1px solid #E8E4DC", overflow: "hidden", transition: "all 0.15s" }}>
        <div style={{ padding: "14px 18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
            <div style={{ flex: 1, minWidth: 0, cursor: "pointer", display: "flex", alignItems: "flex-start", gap: 8 }} onClick={() => toggleCard(r.id)}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13.5, fontWeight: 600, color: "#1C2B24", lineHeight: 1.4, marginBottom: 4 }}>{r.title}</div>
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, color: "#8A8880", lineHeight: 1.4 }}>
                  {r.authors}{r.year ? ` · ${r.year}` : ""}{r.journal ? ` · ${r.journal}` : ""}
                </div>
              </div>
              <div style={{ flexShrink: 0, marginTop: 2, color: "#C2BEB6" }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
              <button onClick={(e) => { e.stopPropagation(); toggleBookmark(r); }} title={marked ? "Remove bookmark" : "Bookmark article"}
                style={{ width: 32, height: 32, borderRadius: 8, border: "none", background: marked ? `${accent}15` : "#F5F3EE", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}>
                {marked
                  ? <BookmarkCheck size={15} color={accent} strokeWidth={2.2} />
                  : <Bookmark size={15} color="#AAA" strokeWidth={1.8} />}
              </button>
  ...
  return (
    <div style={{ fontFamily: "'Source Serif 4','Georgia',serif", background: "#F7F6F1", minHeight: "100vh", color: "#1C2B24" }}>
      <style>{`
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #EDEBE4", padding: "20px 28px" }}>
        <div style={{ maxWidth: 920, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <div>
            <h1 style={{ fontFamily: "'Source Serif 4',serif", fontSize: 22, fontWeight: 800, color: "#1C2B24", margin: 0, letterSpacing: "-0.3px" }}>Search</h1>
            <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, color: "#A0A89E", margin: "2px 0 0", letterSpacing: "0.2px" }}>8 databases · AI synthesis · birth to 18</p>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            {synthesis && (
              <button onClick={() => exportSynthesisPDF(synthesis, searchMeta, bookmarks)}
            ...
            {(synthesis || Object.keys(sourceResults).length > 0) && (
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "28px 16px 80px" }}>
              ...
              {synthesis.suggested_searches?.length > 0 && (
                <div style={{ marginTop: 16 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {synthesis.suggested_searches.map((s: string, i: number) => <button key={i} onClick={() => { setQuery(s); setTimeout(() => handleSearch(), 0); }} style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, padding: "6px 14px", borderRadius: 8, border: `1px solid ${accent}33`, background: `${accent}08`, color: accent, cursor: "pointer", fontWeight: 500 }}>{s}</button>)}
                  </div>
                </div>
              )}
            </>)}
        ...
        <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: "#BBB", marginTop: 36, fontStyle: "italic", lineHeight: 1.6 }}>
          Seedling aggregates PubMed, Semantic Scholar, OpenAlex, ERIC, ClinicalTrials.gov, Europe PMC, CORE & CrossRef. AI synthesis supports clinical reasoning — always verify against current literature and professional judgement. Not a substitute for clinical evaluation.
        </p>
      </div>
    </div>
  );
